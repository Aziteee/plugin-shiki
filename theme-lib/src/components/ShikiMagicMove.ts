import type { HighlighterCore, StringLiteralUnion, ThemeRegistrationAny } from 'shiki/core'
...
export class ShikiMagicMove extends HTMLElement {
  ...
  private _themes!: Partial<Record<string, ThemeRegistrationAny | StringLiteralUnion<string, string>>>
  ...
  set themes(value: Partial<Record<string, ThemeRegistrationAny | StringLiteralUnion<string, string>>>) {
    this._themes = value
    this.propertyChangedCallback()
  }
  ...
  updateRenderer() {
    if (this.machine && this.renderer) {
      ...
      this.renderer.highlightedDOM = this.highlighter.codeToHtml("", {
        lang: this.lang,
        themes: this.themes,
      })
    }
