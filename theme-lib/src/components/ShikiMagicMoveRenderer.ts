...
import { MagicMoveRenderer as Renderer } from 'shiki-magic-move/renderer'
...
export class ShikiMagicMoveRenderer extends HTMLElement {
  ...
  private _highlightedDOM: string = ''
  ...
  get highlightedDOM() {
    return this._highlightedDOM
  }

  set highlightedDOM(value: string) {
    this._highlightedDOM = value
  }
  ...
  connectedCallback() {
    this.innerHTML = this.highlightedDOM
    const preEl = this.querySelector('pre')!
    preEl.className += ` ${this.class}`

    this.container = this.querySelector('code')!
